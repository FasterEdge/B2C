...
- 分析結果は、[ファイルシンクプラグイン](docs/en_US/guide/sinks/builtin/file.md)を使用してローカルファイルシステムに書き込まれます。
...
リリースアセットには、事前にビルドされたバイナリファイルが提供されています。事前にビルドされたバイナリファイルがないOSまたはアーキテクチャを使用している場合は、クロスコンパイルを使用して自分でビルドしてください。詳細については、[ドキュメント](docs/en_US/index.md)を参照してください。

コンパイル中に、goビルドタグを選択して、必要な機能のみを含むカスタマイズされた製品をビルドし、バイナリサイズを削減することができます。ターゲットの展開環境にリソース制約がある場合、パッケージのサイズは特に重要です。詳細については、[ドキュメント](docs/en_US/index.md)を参照してください。
